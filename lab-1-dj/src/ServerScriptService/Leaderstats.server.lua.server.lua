...
advanceEvent.OnServerEvent:Connect(function()
	...
	-- Check if any player is still near the first segment before destroying it
	if #segments > SEGMENTS_AHEAD + 2 then
		local firstSeg = segments[1]
		if firstSeg then
			local segZ = firstSeg.PrimaryPart.Position.Z
			local canDestroy = true
			
			-- Check if any player is within 50 studs of this segment
			for _, player in ipairs(Players:GetPlayers()) do
				local char = player.Character
				if char then
					local hrp = char:FindFirstChild("HumanoidRootPart")
					if hrp then
						local dist = math.abs(hrp.Position.Z - segZ)
						if dist < 50 then  -- Keep segment if player is close
							canDestroy = false
							break
						end
					end
